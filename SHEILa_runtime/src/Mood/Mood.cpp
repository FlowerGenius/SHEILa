...
std::string Mood::getFeeling(){
	...
	for(std::vector<Feeling>::iterator it = server::feelings.begin(); it != server::feelings.end(); ++it) {
		bool match = true;
		...
		if ( !(*it).anger_range.contains(getAngerStrength()) ) {
			...
			match = false;
		} else if ( !(*it).anticipation_range.contains(getAnticipationStrength()) ) {
			match = false;
		} else {
			match = true;
	...
	std::cout << "All Feelings: " << server::feelings.size() << std::endl << "Matches: " << allfeelings.size() << std::endl;
